...
import { collectLiterals } from "../../evidence/literals.js";
...
export async function validateEnumerations(
  ...
  for (const enumeration of config.enumerations) {
    ...
    const evidence = await collectLiterals(
      reader,
      files,
      enumeration.values.sources,
      enumeration.values.pattern,
    );
...
function checkDocument(
  ...
  evidence: Awaited<ReturnType<typeof collectLiterals>>,
): Finding[] {
    ...
    return [
      {
        ...
        severity: "error",
        message: `No file matches the ${enumeration.label} sources ${enumeration.values.sources.join(", ")}.`,
        document: start,
        suggestion: "Point the enumeration at source files that exist.",
      },
    ];
  }
  ...
  for (const [value, span] of documented) {
    ...
    findings.push({
      ...
      severity: "error",
      message: `Documented ${enumeration.label} "${value}" is not defined in ${enumeration.values.sources.join(", ")}.`,
